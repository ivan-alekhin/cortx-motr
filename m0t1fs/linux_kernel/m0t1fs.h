...
#ifndef __MOTR_M0T1FS_M0T1FS_H__
#define __MOTR_M0T1FS_M0T1FS_H__
...
/**
  ...
  m0t1fs is a motr client file-system for linux. It is implemented as a
  kernel module.
  ...
  - fid_start [value type: uint32_t, default: 4]
      ...
      ".motr", ".motr/fid" and potentially many others that will live in ".motr"
      directory.
...
/**
  ...
  them in order to run lustre on top of motr.
 ...
 * @{
 *
 * @page motr-cmd-HLD-DLD Motr Clustered Meta-data HLD and DLD
 *
 * - @ref Motr-CMD-ovw
 * - @ref Motr-CMD-def
 * - @ref Motr-CMD-req
 * - @ref Motr-CMD-depends
 * - @ref Motr-CMD-highlights
 * - @ref Motr-CMD-fspec
 * - @ref Motr-CMD-lspec
 *   - @ref Motr-CMD-lspec-schema
 * - @ref Motr-CMD-conformance
 * - @ref Motr-CMD-ut
 * - @ref Motr-CMD-st
 * - @ref Motr-CMD-O
 * - @ref Motr-CMD-ref
 *
 * <HR>
 * @section Motr-CMD-ovw Overview
 * This document contains the high level and detailed level design for the Motr
 * Clustered Meta-data (a.k.a CMD).
 *
 * More than a single meta-data server will be supported in Motr CMD, scattering
 * meta-data and meta-data operations across all Motr server nodes. Multiple
 * mdservices will be running in a Motr system.
 *
 ...
 *
 * @section Motr-CMD-def Definitions
 *  - CMD: Clustered Meta-data. With CMD, meta-data of Motr is stored across
 ...
 *  - AA:  Active Archive. Motr client is used in some archive system. Objects
 *         from archive system are stored in Motr client as a file in root
 *         directory.
 *  - FID: File IDentification. FID is used to identify an object uniquely in
 *         Motr.
 *  - confc/confd: Motr configuration client and configuration daemon.
 *         Motr configuration is system information about how a Motr system
 *         is configured. This information is used to configure and start
 *         various Motr services, networks, transports, clients, etc.
 *
 * @section Motr-CMD-req Requirements
 * The specified requirements are as follows:
 * - R.CMD: Motr system may run mdservices on multiple server nodes. Motr
 *          meta-data are stored across all these mdservices. Motr meta-data
 ...
 * - R.CMD.conf: The configuration of mdservices is managed by confd. A Motr
 *               client gets this configuration by confc.
 * - R.CMD.namespace: Objects (files) are all stored root directory. A Motr
 ...
 * - R.CMD.oostore: A oostore mode for motr client is needed. In this mode,
 ...
 *
 * @section Motr-CMD-depends Dependency
 * This feature has no dependency on others right now.
 *
 *
 * @section Motr-CMD-highlights Highlights
 * N/A
 *
 * @section Motr-CMD-fspec Functional Specification
 * To achieve the requirements in @ref Motr-CMD-req step by step, the CMD
 ...
 *            An algorithm will be used to map a fid to mdservice. So Motr client
 ...
 *                   unnecessary.
 * - Stage RM-CLIENT: In this stage, Motr clients start using RM for fid
 *                    allocation. There is a special single instance of RM
 *                    service, that manages the entire fid name-space
 *                    (0, 0)---(~0ULL, ~0ULL). Motr clients request ranges of
 ...
 *
 * @section Motr-CMD-lspec Logical Specification
 ...
 * mdservice. Motr client has a "struct m0_reqh_service_ctx" for every
 ...
 * is not supported in Motr. Link operation will be disabled after this
 * feature is landned.
 *
 ...
 * temporary mode, OOSTORE mode of Motr client, and in this mode the FID of
 ...
 *
 * @subsection Motr-CMD-lspec-schema Schema
 * Configuration of multiple mdservices is managed by confd, and provided to
 * Motr client via confc. The schema for mdservice is similar to that for
 * ioservice. Motr client startup code will parse this configuration and setup
 * mdservice and mapping array.
 *
 * @section Motr-CMD-conformance Conformance
 * - I.CMD: Multiple mdservices can be started in a Motr system, and clients
 *          can connect to these mdservices. Motr clients map objects to proper
 *          mdservice. Meta-data operations go to the right mdservice.
 * - I.CMD.share: Motr mdservice shares nothing, and is independent.
 * - I.CMD.conf: The configuration of mdservices is managed by confd. The
 *   		 mdservice schema is similar to that of ioservice. Motr clients
 ...
 * - I.CMD.code-reuse: No particular workaround is used to gather the directory
 *                     entries for root dir. All code will be reusable for
 *                     non-root directory.
 * - I.CMD.oostore: A oostore mode for motr client is added. In this mode,
 ...
 *
 * @section Motr-CMD-ut Unit Tests
 * Existing unit test are still valid and useful. The following unit tests are
 * added to verify new functionalities.
 *
 * Unit Test 1: Good configuration for mdservice. Motr client starts correctly.
 * Unit Test 2: Invalid configurations for mdservice. Motr client reports error
 *              and failed to start.
 ...
 *
 * @section Motr-CMD-st System Tests
 ...
 * System Test 2: Multiple mdservices are configured in confd, and Motr clients
 *                can start up correctly.
 *
 * @section Motr-CMD-O Performance
 * N/A
 *
 * @section Motr-CMD-ref References
 * N/A
 *
 * @}
 */
...
struct m0t1fs_sb {
	...
	/** /.motr virtual dir dentry */
	struct dentry                          *csb_motr_dentry;

	/** /.motr/fid virtual dir dentry */
...
#endif /* __MOTR_M0T1FS_M0T1FS_H__ */
